<template>
    <Head>
        <title>{{ $page.props.setting.app_name ?? 'Atur Setting Terlebih Dahulu' }}  - Edit Kategori Peminatan</title>
    </Head>
    <!--start page wrapper -->
                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Edit Kategori Peminatan</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    export default {
        ...
        setup(props) {
            const form = reactive({
                ...
                thumbnail: '',
                development_status: props.category.development_status,
            });

            // submit method
            const submit = () => {
                // send data to server
                Inertia.post(`/admin/categories/${props.category.id}`, {
                    forceFormData: true,
                    _method: 'put',
                    // data
                    name: form.name,
